   ...
   public class UI2 extends Sprite
   {
      ...
      public function updateSelectorBricks() : void
      {
         ...
         while(_loc3_ < _loc2_.length)
         {
            ...
            for(; _loc9_ < _loc4_.bricks.length; _loc9_++)
            {
               ...
               if(_loc11_.payvaultid == "" || this.base.client.payVault.has(_loc11_.payvaultid) || _loc11_.payvaultid == "pro" && Global.player_is_beta_member || _loc11_.payvaultid == "goldmember" && Global.playerObject.goldmember)
               {
                  if(!((_loc11_.id == 77 || _loc11_.id == 83 || _loc11_.id == 1520) && !Global.hasOwner))
                  {
                     ...
                     {
                        if(!_loc11_.requiresOwnership || (Bl.data.owner || Bl.data.isAdmin))
                        {
                           switch(_loc11_.tab)
                           {
                              case ItemTab.BLOCK:
            ...
            if(_loc5_.length > 0)
            {
               _loc10_ = new BrickPackage(_loc4_.name,_loc5_,this,ItemTab.BLOCK,_loc4_.tags,Global.blockPackageTitlesVisible,!!Global.blockSelectorCollapsedMode?1:0);
               this.bselector.addPackage(_loc10_);
            }
            if(_loc6_.length > 0)
            {
               _loc10_ = new BrickPackage(_loc4_.name,_loc6_,this,ItemTab.ACTION,_loc4_.tags,Global.blockPackageTitlesVisible,!!Global.blockSelectorCollapsedMode?1:0);
               this.bselector.addPackage(_loc10_);
            }
            if(_loc7_.length > 0)
            {
               _loc10_ = new BrickPackage(_loc4_.name,_loc7_,this,ItemTab.DECORATIVE,_loc4_.tags,Global.blockPackageTitlesVisible,!!Global.blockSelectorCollapsedMode?1:0);
               this.bselector.addPackage(_loc10_);
            }
            if(_loc8_.length > 0)
            {
               _loc10_ = new BrickPackage(_loc4_.name,_loc8_,this,ItemTab.BACKGROUND,_loc4_.tags,Global.blockPackageTitlesVisible,!!Global.blockSelectorCollapsedMode?1:0);
               this.bselector.addPackage(_loc10_);
            }
            _loc3_++;
         }
      ...
      private function handleKeyDown(param1:KeyboardEvent) : void
      {
         ...
         {
            if(param1.keyCode == 81 && this.showProperties)
            {
               this.specialproperties.decrementValue(1);
            }
            if(param1.keyCode == 69 && this.showProperties)
            {
